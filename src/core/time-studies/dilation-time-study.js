...
export class DilationTimeStudyState extends TimeStudyState {
  ...
  get totalTimeTheoremRequirement() {
    return this.id === 1 ? (Pelle.isDoomed ? 21900 : 12900) : 0;
  }
  ...
  purchase(quiet = false) {
    ...
    if (this.id === 1) {
      ...
      if (!Pelle.isDoomed) Currency.tachyonParticles.bumpTo(
        Decimal.max(Perk.startTP.effectOrDefault(0), Achievement(104).enhancedEffect.effectOrDefault(0)));
